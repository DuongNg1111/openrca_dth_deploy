...
Input: User failure report through mail (message/text)

Example: "Cannot create shipping order at 17:23:19."

**2. Retrieve Relevant Telemetry**

Retrieve the relevant Logs, Metrics, and Traces based on the user query (e.g., timestamp, service name, or keywords).
This step narrows down the massive telemetry dataset to only the data relevant to the reported incident

**3. Generate Python Programs**

The LLM generates four specialized AI agents, each with a specific responsibility:

*1. Log Agent* – Generates lightweight Python scripts to analyze logs and detect suspicious events.

*2. Metric Agent*– Generates lightweight Python scripts to analyze CPU, Memory, Latency, and other KPIs.

*3. Trace Agent* – Generates lightweight Python scripts to analyze request flows and service dependencies.

*4. Reasoning Agent* – Collects and evaluates the evidence from the other three agents to determine whether it is sufficient for root cause analysis.

**4. Execute & Analyze**

Execute the Python scripts generated by the Log, Metric, and Trace Agents.

Collect investigation results and send them to the Reasoning Agent.


**5. Iterative Reasoning**

If the evidence is insufficient, the Reasoning Agent instructs the LLM to generate another round of lightweight Python scripts for deeper analysis.

Repeat until sufficient evidence is collected.
